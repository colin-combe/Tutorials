...
```{r}
OCCMscores <- read.csv("~/Tutorials/ggplot/OCCMscores.csv")
head(OCCMscores)
```
...
```{r}
p <- ggplot(data=OCCMscores, aes(x=Self, y=Score) )
```
...
```{r}
h<- ggplot(OCCMscores, aes(Score, fill=Self )) +geom_histogram(binwidth=0.1)
h
```
...
```{r}
p <- p +  
  geom_violin( aes( x = Self, y = Score ) )
p
```
...
```{r}
h + facet_wrap(~DecoyType, ncol=1)
```


```{r}
p + facet_wrap(~DecoyType, ncol=1)
```
